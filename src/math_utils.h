...
inline real_number atan_approximation(const real_number& x) {
    const real_number a1  =  0.99997726;
    const real_number a3  = -0.33262347;
    const real_number a5  =  0.19354346;
    const real_number a7  = -0.11643287;
    const real_number a9  =  0.05265332;
    const real_number a11 = -0.01172120;
...
real_number atan2_custom(const real_number y, const real_number x) {
        ...
        res = swap ? (atan_input >= 0.0f ? M_PI_2 : -M_PI_2) - res : res; //TODO: NOT WORKING OPTIMIZATION. BE CAREFUL WITH y=-1 and x=0 for example
