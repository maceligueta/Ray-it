...
std::pair<real_number, real_number> Box::Intersect(Ray &r) const{
    ...
    if ((tmin > tymax) || (tymin > tmax)){
        return std::pair<real_number,real_number>(real_number(0.0), real_number(0.0));
    }
    if (tymin > tmin)
        tmin = tymin;
    if (tymax < tmax)
    ...
    if (tzmax < tmax)
        tmax = tzmax;
    return  std::pair<real_number,real_number>(tmin, tmax);
}